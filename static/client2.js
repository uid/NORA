...
//  *************************************************  login stage *************************************************
    ...
    now.startup = function(username, gameID, s, e) {
        ...
        $('#source').text(s);
        $('#excerpt').text(e);
        $('#generateParaphrase').css('display', '');
...
//                       '<span style=background-color:'+background+';width:100%;display:block;overflow:hidden;>'+tag + '</span></div>';
            ...
            var msgntagsStart = '<div class=msgntags id=mt_'+msg_id+' style="opacity: '+opacity+'" data-newthread="1">'
...
//                        $('#paraphraseSelected').html('<span class="paraSelected" id="paraSelectedText">'+text+'</span>');
            ...
            $('#mt_'+msg_id).droppable({
                hoverClass: "highlight",
                drop: function(event, ui) {
//                    console.log("event (target) "+event.target.id);
//                    console.log("ui (source) "+ui.draggable[0].id);
                    
                    now.serverMergeThread(myUsername, myGameID, ui.draggable[0].id, event.target.id);
                }
            });
            ...
            thread.droppable({
                drop: function(event, ui) {
//                    console.log("event drop thread (target) "+event.target.id); //t_denzil_p1m2
//                    console.log("ui drop thread (source) "+ui.draggable[0].id); //mt_denzil_p1m3 
                    $('#'+ui.draggable[0].id).attr('data-newthread', '0');
                }
           });
        
            $('#mt_'+msg_id).draggable({
                stop: function(event, ui) {
//                    console.log("event drag (target) "+event.target.id);
//                    console.log("ui drag (source) ");
//                    console.log(ui);
//                    
                    var offset = Math.abs(parseInt($('#'+event.target.id).css('left')));
                    var offsetTop = Math.abs(parseInt($('#'+event.target.id).css('top')));
                    var offsetHeight = Math.abs(parseInt($('#'+event.target.id).parent().css('height')));
                    
                    var source = $('#'+event.target.id);
                    var newThread = source.attr('data-newthread');
                    if(newThread=="1") {
                        if (offset > 100 || offsetTop > offsetHeight) {
                            //create new thread with msg
                            if($('#'+event.target.id).parent().attr('data-size')=="1") {
                                $('#'+event.target.id).css('top',0);
                                $('#'+event.target.id).css('left',0);
                            }
                            else {
                                var un = source.attr('id').substring(3,source.attr('id').indexOf('_',3));
                                var gameID = $('span#gameID').text();
                                var cm = $('#'+event.target.id+' .msg').text().substring(un.length+2);
                                var tag = $('#'+event.target.id+' #tagsnlike').text().substring(1, $('#'+event.target.id+' #tagsnlike').text().indexOf('+'));
                                var paraphrase = event.target.id.substring(source.attr('id').indexOf('_',3)+2,source.attr('id').indexOf('m',source.attr('id').indexOf('_',3)));
                                var st = "";
                                var numLikes = parseInt($('#'+event.target.id+' .plusOne').text().substring(2));
                                var parentSource = source.parent().attr("id");
                                $('#'+parentSource).attr('data-size', parseInt($('#'+parentSource).attr('data-size'))-1);
                                if ($('#'+parentSource).attr('data-size') == "0") { 
                                    $('#'+parentSource).remove(); 
                                }
                                source.remove();
                                now.serverRemoveItem(un, gameID, event.target.id);
                                now.serverGotNewChat(un, gameID, cm, tag, paraphrase, st, numLikes)
                            }
                        }
                        else {
                            var source = event.target.id;
    ...
    now.addMsg = function(username, gameID, msgid, msg_id, chatText, numLikes){
        ...
        if (myGameID == gameID) {
            ...
            var tag = $('#mt_'+msg_id+' #tagsnlike').text().substring(1, $('#mt_'+msg_id+' #tagsnlike').text().indexOf('+'));
            var sentenceNum = document.getElementById('sourceReview').getAttribute('data-paraphraseActive'); 
//            $('#mt_'+msg_id).attr('id').substring($('#mt_'+msg_id).attr('id').indexOf('_',3)+2,$('#mt_'+msg_id).attr('id').indexOf('m',$('#mt_'+msg_id).attr('id').indexOf('_',3)));
...
//                        '<span style="background-color:'+background+';width:100%;display:block; overflow:hidden;">'+'</span></div>';
            ...
            now.logCSV();
            var msgntagsStart = '<div class=msgntags id=mt_'+msgid+' style="opacity: '+opacity+'" data-newthread="1" data-tagOriginal="'+tag+'" data-sentenceNum="'+sentenceNum+'">';
...
//                    
                    ...
                    if(newThread=="1") {
                        if (offset > 100 || offsetTop > offsetHeight) {
                            ...
                            if($('#'+event.target.id).parent().attr('data-size')=="1") {
                                ...
                                var un = source.attr('id').substring(3,source.attr('id').indexOf('_',3));
                                var gameID = $('span#gameID').text();
                                var cm = $('#'+event.target.id+' .msg').text().substring(un.length+3);
                                var tag = source.attr('data-tagOriginal');
                                var paraphrase = source.attr('data-sentenceNum');
                                var st = "";
                                var numLikes = parseInt($('#'+event.target.id+' .plusOne').text().substring(2));
                                var parentSource = source.parent().attr("id");
                                $('#'+parentSource).attr('data-size', parseInt($('#'+parentSource).attr('data-size'))-1);
    ...
    now.removeItem = function(gameID, item) {
        var myGameID = $('span#gameID').text();
        if (myGameID == gameID) {
            var myGameID = $('span#gameID').text();
            if($('#'+item).length == "1") {
    ...
    now.mergeThread = function(username, gameID, source, target) {
        var myGameID = $('span#gameID').text();
        var myUsername = $('span#username').text();
        if (myGameID == gameID) {
            now.logCSV();
...
//            console.log("source "+source+" parent "+parentSource);
//            console.log("target "+target+" parent "+parentTarget);
            
            chatbox = $('#chatbox'+parentTarget.substring(1));
//            chatbox.remove();
//            $('#'+parentTarget).append($('#'+source));
//            $('#'+parentTarget).append(chatbox);
            
            $('#'+source).insertBefore($(chatbox));
            $('#'+parentSource).attr('data-size', parseInt($('#'+parentSource).attr('data-size'))-1);
            $('#'+parentTarget).attr('data-size', parseInt($('#'+ parentTarget).attr('data-size'))+1);
            if ($('#'+parentSource).attr('data-size') == "0") { 
                $('#'+parentSource).remove(); 
            }
            $('#'+source).attr('data-newthread', '1');  
...
//    });
    ...
    now.newMessage = function() {
        ...
        var sentence = document.getElementById('sourceReview').getAttribute('data-paraphraseActive');
        if (sentence != "") {
            var username = $('span#username').text();
            var gameID = $('span#gameID').text();
...
//                console.log("sentence 0 or 1 indexed? "+sentence); it's 1 indexed
                now.serverGotNewChat(username, gameID, chatMsg, tag, sentence, selectedText, 0);
                var scoreAdd = "10";
...
//  *************************************************  paraphrase stage *************************************************
    ...
    now.paraphraseClicked = function (elem, num) {
        var currentNum=$('#sourceReview').attr("data-paraphraseActive");
        num += 1;
        if(num!=currentNum) {
...
//            var newLeft = parseInt($('#leftCol').css('width'))+10;
            ...
            document.getElementById('sourceReview').setAttribute('data-paraphraseActive', num);
...
//                $('#paraphraseSelected').html('<span class="paraSelected" id="paraSelectedText">'+text+'</span>');
    ...
    now.paraphraseStage = function(username, gameID, usersScoreInGame, s, e) {
        ...
        $('#sourceReview').text(s);
        $('#excerptReview').text(e);
        if(gameID.toLowerCase().substring(0, 7) == "spanish") {
            $('#excerptReview').css('display','none');
            $('#paraphraseSection').css('height', '95%');
        }
        $('#generateParaphrase').css('display', 'none');
        $('#reviewParaphrase').css('display', '');
        if(gameID.toLowerCase().substring(0,7) == "spanish") 
...
//        
        ...
        $('#commentBoxMinimized').click(function() {
            $('#commentBoxMinimized').css('display','none');
            $('#commentBox').css('left', $('#leftCol').css('width')+20);
            $('#commentBox').css('display', '');
        });
...
//                var parameter = [username, gameID, "moved message", msgID, pos];
                ...
                var num = log[each][3]-1;
                $('#paraphrase'+num).attr('data-original', "false");
            }
            else if( log[each][1] == gameID && log[each][2] == "remove item" ) {
                //var parameter = [username, gameID, "merged threads", threadSource, threadTarget];
                now.removeItem(log[each][1], log[each][3]);
            }
